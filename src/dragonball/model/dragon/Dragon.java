...
public class Dragon implements Serializable {
	...
	public DragonWish[] getWishes() {
		...
		if (senzuBeans > 0) {
			wishes.add(new DragonWish(this, DragonWishType.SENZU_BEANS,
					senzuBeans));
		}

		if (abilityPoints > 0) {
			wishes.add(new DragonWish(this, DragonWishType.ABILITY_POINTS,
					abilityPoints));
		}

		if (superAttacks.size() > 0) {
			SuperAttack randomSuperAttack = superAttacks.get(new Random()
					.nextInt(superAttacks.size()));
			wishes.add(new DragonWish(this, DragonWishType.SUPER_ATTACK,
					randomSuperAttack));
		}

		if (ultimateAttacks.size() > 0) {
			UltimateAttack randomUltimateAttack = ultimateAttacks
					.get(new Random().nextInt(ultimateAttacks.size()));
			wishes.add(new DragonWish(this, DragonWishType.ULTIMATE_ATTACK,
					randomUltimateAttack));
		}
